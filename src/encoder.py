...
def encode_msg_type1(v):
    ...
    bits += format(1 if safe_int(v.get("pac")) else 0, "01b")
...
def encode_msg_type5(v):
    ...
    # DIRECT MAPPING: Pulling lowercase 'a', 'b', 'c', 'd' from dictionary
    # If your parser doesn't provide these, they will be 0.
    a_offset = safe_int(v.get("a"), 0)
    b_offset = safe_int(v.get("b"), 0)
    c_offset = safe_int(v.get("c"), 0)
    d_offset = safe_int(v.get("d"), 0)

    bits =  format(5, "06b")                           
    bits += "00"                                       
    bits += format(mmsi, "030b")                       
    bits += "00"                                       
    bits += format(safe_int(v.get("imo"), 0), "030b")  
    bits += sixbit_ascii(v.get("callsign"), 7)         
    bits += sixbit_ascii(v.get("name"), 20)            
    bits += format(safe_int(v.get("type"), 0), "08b")  
    
    # Dimensions (A, B, C, D)
    bits += format(min(a_offset, 511), "09b")          
    bits += format(min(b_offset, 511), "09b")          
    bits += format(min(c_offset, 63), "06b")           
    bits += format(min(d_offset, 63), "06b")           
